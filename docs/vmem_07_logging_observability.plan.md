...
| Sink | 格式 | 用途 | 启用场景 |
|------|------|------|---------|
| Console (Stdout) | 彩色文本 | 开发调试、CLI 模式 | 始终 |
| File (JSON) | 结构化 JSON，一行一条 | **AI 解析的主要数据源** | 始终 |
| File (Text) | 人类可读文本 | 人类排错 | 始终 |
| Windows EventLog | 系统事件 | Service 故障排查 | Service 模式 |
| Seq / OpenTelemetry | OTLP | 未来扩展（集中式可观测性） | 可选 |
...
| 场景 | 控制台 | JSON 文件 | 文本文件 |
|------|--------|----------|---------|
| 开发 (Debug) | `DBG` | `DBG` | `DBG` |
| 生产 (Release) | `INF` | `INF` | `INF` |
| 排错 (Troubleshoot) | `DBG` | `VRB` | `DBG` |

可通过配置文件或 IPC 命令**运行时动态切换**日志级别，无需重启。
...
每次 WinFsp 回调入口生成一个 `CorrelationId`（8 位 hex），通过 `AsyncLocal<string>` 在整个调用链中传播：
...
public static class OperationContext
{
    ...
    /// WinFsp 回调入口调用，生成新的 correlationId
    public static IDisposable BeginOperation(string callbackName)
    {
        var id = Guid.NewGuid().ToString("N")[..8];
        _correlationId.Value = id;
        Log.Information("FS callback {Callback} started", callbackName);
